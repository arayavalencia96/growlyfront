...
function isSessionService(
  ...
  return (
    "save" in provider && "expire" in provider && "getRefreshToken" in provider
  );
}
...
export class HttpService implements IHttpClient {
  ...
  constructor(
    baseUrl: string,
    authorizationHeadersProvider: IAuthorizationHeadersProvider = new AnonymousAuthorizationHeadersProvider(),
  ) {
